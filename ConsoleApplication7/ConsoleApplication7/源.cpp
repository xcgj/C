...
int getalloc(char **origin, char ***buf, char c)//分配一维二维内存空间
{
	if (origin == NULL || buf == NULL)
		return -1;
	char        **temp_p1 = origin; //用来记录始端的指针1
	char         **temp_p2 = origin; //用来切割的指针2
	char        ***pbuf1 = buf;     //存放子串的指针
	int         line = 0;          //计算有效行数
	while (*temp_p2)              //遍历数组
	{
		*temp_p2 = strchr(*temp_p2, c);//两个指针一开始都计算有效行数
...
int main(void)
{
	char		*oristr = ",avdf,,sdf,,,add,fdf,asf ssdsf,safds";
